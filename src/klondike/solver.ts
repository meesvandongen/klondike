...
import { SUIT_COLOR, rankValue } from "../shared/deck";
import { newState, type KlondikeState } from "./game";
...
type Move =
  | { t: "deal"; prio: number }
  | { t: "w2f"; f: number; prio: number }
  | { t: "t2f"; c: number; f: number; prio: number }
  | { t: "w2t"; to: number; prio: number }
  | { t: "t2t"; from: number; to: number; start: number; prio: number };
...
function cloneState(s: KlondikeState): SolverState {
  return {
...
function totalFoundation(state: SolverState): number {
  let n = 0;
  for (const p of state.foundations) n += p.length;
  return n;
}

function won(state: SolverState): boolean {
...
function collectMoves(state: SolverState): Move[] {
  const moves: Move[] = [];
  ...
  for (let from = 0; from < 7; from++) {
    ...
    for (let start = firstUp; start < col.length; start++) {
      ...
      for (let to = 0; to < 7; to++) {
        ...
        if (state.tableau[to].length === 0 && start === firstUp && firstUp === 0)
          continue;
...
function applyMove(state: SolverState, m: Move): () => void {
  switch (m.t) {
    case "deal": {
...
export function isSolvable(
  ...
  opts: { maxNodes?: number; maxTimeMs?: number } = {},
): SolveResult {
  ...
  function recurse(): boolean {
    ...
    for (const m of moves) {
      const undoFn = applyMove(state, m);
      const ok = recurse();
      undoFn();
      if (ok) return true;
      if (timedOut) return false;
...
export interface SolvableDeal {
  state: KlondikeState;
  attempts: number;
  verified: boolean;
}

export function findSolvableState(
  opts: {
    draw?: number;
    totalBudgetMs?: number;
    perAttemptMs?: number;
    perAttemptNodes?: number;
  } = {},
): SolvableDeal {
  ...
  const start = Date.now();
  let lastState: KlondikeState = newState({ draw });
  let attempts = 0;
